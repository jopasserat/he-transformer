...
NGRAPH_TEST(${BACKEND_NAME}, pad_exterior_1d) {
  ...
  EXPECT_TRUE(test::all_close_f(
      (test::NDArray<float, 1>({2112, 2112, 2112, 2112, 1, 2, 3, 4, 5, 6, 2112,
                                2112, 2112, 2112, 2112})
           .get_vector()),
...
NGRAPH_TEST(${BACKEND_NAME}, pad_edge_1d_top_neg) {
  ...
  handle->call_with_validate({result}, {a, b});
  EXPECT_TRUE(
      test::all_close_f((test::NDArray<float, 1>({1, 1, 1, 2, 3}).get_vector()),
                        read_vector<float>(result), MIN_FLOAT_TOLERANCE_BITS));
}
...
NGRAPH_TEST(${BACKEND_NAME}, pad_reflect_1d_top_neg) {
  ...
  handle->call_with_validate({result}, {a, b});
  EXPECT_TRUE(
      test::all_close_f((test::NDArray<float, 1>({3, 2, 1, 2, 3}).get_vector()),
                        read_vector<float>(result), MIN_FLOAT_TOLERANCE_BITS));
}
